...
class EditAvatar extends Component {
    ...
    componentDidMount () {
        ...
        let cropBoxData, canvasData, cropper, edit_avatar_wrap, edit_avatar_img;
        $('#edit-avatar').on('hidden.bs.modal', () => {
            cropper.reset();
            cropBoxData = cropper.getCropBoxData();
            canvasData  = cropper.getCanvasData();
            cropper.destroy();
            edit_avatar_img.remove();
        });
        $('#edit-avatar').on('shown.bs.modal', () => {
            edit_avatar_wrap    = document.querySelector('#edit-avatar .photo-edit-item.editor');
            edit_avatar_img     = document.createElement("img");
            edit_avatar_img.src = self.props.profile.temp_avatar;
            edit_avatar_wrap.appendChild(edit_avatar_img);
            cropper = new Cropper(edit_avatar_img, {
                dragMode: 'move', autoCropArea: .9, checkCrossOrigin: false, checkOrientation: false, aspectRatio: 1/1,
                crop(e) { 
        ...
        $('#avatar-editor-ctrl button').on('click', function (e) { 
            let data = $(this).data(); 
            if (data.method == 'zoom') { 
        ...
        document.querySelector('#crop-avatar-done').addEventListener('click', (e) => {
            submit.status = true;
            ...
            cropper.getCroppedCanvas().toBlob((blob) => {
                ...
                formData.append('old', this.props.profile.header.avatar);
                ...
                xmlhttp.open('post', Config.API.URL+'/user/avatar/save');
    ...
    render () {
        return (
            <div class="modal fade" id="edit-avatar" tabIndex="-1" role="dialog" aria-labelledby="update-status" aria-hidden="true">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="photo-edit-wrap" id="photo-edit-wrap">
                                <div class="photo-edit-item editor-ctrl" id="avatar-editor-ctrl">
                            ...
                            <div class="modal-footer">
                                <button class="btn btn-purple btn-lg full-width" disabled={this.state.submit.status} id="crop-avatar-done">{this.state.submit.label}</button>
                            </div>
                        </div>
                    </div>
                </div>
